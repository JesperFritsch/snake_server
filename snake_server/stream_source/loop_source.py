...
class AsyncIPCLoopSource(ILiveStreamSource):
    def __init__(self, pipe: Connection, run_config: dict, expansion: int = 2):
        super(ILiveStreamSource, self).__init__()
        self._run_config = run_config
        self._pipe = pipe
        self._loop_observable = IPCRepeaterObservable(self._pipe)
        self._state_builder = StateBuilderObserver()
        self._expansion = expansion
        self._map_builder = MapBuilderObserver(expansion=expansion)
        self._loop_observable.add_observer(self._state_builder)
        self._loop_observable.add_observer(self._map_builder)
        self._loop_observable.start()
        
    def is_done(self):
        return self._loop_observable.is_done()

    def last_available_step(self):
        return self._state_builder._steps[-1].step if self._state_builder._steps else -1

    def get_run_config(self):
        return self._run_config

    def get_step_range(self, start: int, end: int) -> List[CompleteStepState]:
        steps = [self._state_builder.get_state(i) for i in range(start, end + 1)]
        return steps

    def get_map(self, step: int) -> np.ndarray:
        return self._map_builder.get_map_for_step(step)

    def get_map_range(self, start: int, end: int) -> List[List[np.ndarray]]:
        maps = [[self._map_builder.get_map_for_step(i), *[self._map_builder.get_next_map() for _ in range(1, self._expansion)]] for i in range(start, end + 1)]
        return maps

    def get_step(self, step_nr: int) -> CompleteStepState:
        return self._state_builder.get_state(step_nr)

    def get_map_data(self, step_nr: int) -> np.ndarray:
        return self._map_builder.get_map(step_nr)

    async def get_meta_data(self) -> EnvMetaData:
        start_time = time.time()
        while not self._state_builder._start_data and time.time() - start_time < 20:
            await asyncio.sleep(0.05)
        if not self._state_builder._start_data:
            raise RuntimeError('Source is not ready yet')
        return self._state_builder._start_data.env_meta_data
    
    def cancel(self):
        self._loop_observable.stop()
        self._pipe.close()
